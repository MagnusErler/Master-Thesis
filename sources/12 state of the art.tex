...
\ac{LoRa} refers to the physical layer and the proprietary spread spectrum modulation technique developed by Semtech, while \ac{LoRaWAN} refers to the network layer that defines the communication protocol and system architecture, enabling devices to communicate with \ac{LoRa} gateways and network servers. 
...
\begin{figure}[H]
    ...
    \caption{Linear up-chirp signal: (a) Time domain representation. The signal's frequency increases linearly as the time passes; (b) The variation of the frequency of the signal that is illustrated in panel (a). Rework from: \url{https://www.researchgate.net/figure/Linear-up-chirp-signal-a-Time-domain-representation-The-signals-frequency-increases_fig4_341373286}}
...
We want go into much details about how the technology work, but will stick to describing the major points that influence data rate, power usage and range, which is bandwidth and sweep rate/spreading factor.
The bandwidth is the frequency ranges which the signal sweeps over. Increasing the

The higher the bandwidth, the higher the data rate (chip rate)
The lower the bandwidth, the lower the data rate (chip rate)

increasing bandwidth gives a better qualitity signal, but bandwidth is scarse and is restricted. It is restricted to 500 khz, 250 khz and 125 khz.
In europe; 250 khz and 125 khz.
...
Sweep rate: how fast the sweep changes
And the change it sweeps over; the bandwidth,


The faster the sweep rate, the more information can be passed
Slow sweep rate. Easy to decode.
Spreading factor: in eu 7-12.
